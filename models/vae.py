...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            is_second_to_last_block = i == len(block_out_channels) - 2
            ...
            down_block = get_down_block(
                ...
                horizontal_half_compression=horizontal_half_compression and is_second_to_last_block,
            )
...
class VectorQuantizer(nn.Module):
    ...
    def __init__(
        ...
        if self.show_usage:
            self.register_buffer("codebook_used", nn.Parameter(torch.zeros(262144)))
    ...
    def forward(self, z: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, Tuple]:
        if  self.is_column:
            # reshape z -> (batch, width, height, channel) and flatten
        ...
        if self.is_column:
            z_q = z_q.permute(0, 3, 2, 1).contiguous()
        else:
        ...
        return z_q, (loss, codebook_usage), (perplexity, min_encodings, min_encoding_indices)
