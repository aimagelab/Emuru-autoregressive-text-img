...
# Copied from diffusers.models.unets.unet_2d_blocks
...
from .upsampling import KUpsample2D, FirUpsample2D, Upsample2D
...
def get_down_block(
    ...
    horizontal_half_compression: bool = False,
):
    ...
    if down_block_type == "DownBlock2D":
        ...
        return DownEncoderBlock2D(
            ...
            horizontal_half_compression=horizontal_half_compression,
        )
    ...
    def __init__(
        ...
        if add_downsample:
            self.downsamplers = nn.ModuleList(
                [
                    Downsample2D(
                        out_channels, use_conv=True, out_channels=out_channels, padding=downsample_padding, name="op", vertical_total_compression=vertical_total_compression, horizontal_half_compression=horizontal_half_compression
                    )
                ]
            )
        else:
...
class UpDecoderBlock2D(nn.Module):
    def __init__(
        ...
        if add_upsample:
            self.upsamplers = nn.ModuleList([Upsample2D(out_channels, use_conv=True, out_channels=out_channels, vertical_total_compression=vertical_total_compression, horizontal_half_compression=horizontal_half_compression)])
        else:
            self.upsamplers = None
